...
-- Clone both the encoder and the decoder
function create_split_all_clones(checkpoint, max_len)
  local encoder, decoder, opt = checkpoint[1][1], checkpoint[1][2], checkpoint[2]
  ...
  decoder:replace(function(module)
    if torch.typename(module) == 'nn.LookupTable' then
      local weight = module.weight
      local layer = nn.Linear(weight:size(1), weight:size(2), false)
  ...
  return {
    ['lookup'] = lookuptable_module,
    ['linear'] = linear_module,
    ['encoder_clones'] = clone_many_times(encoder, max_len),
    ['decoder_clones'] = clone_many_times(decoder, max_len),
    ['opt'] = opt
  }
end
...
function main()
  ...
  -- Decoder side tokenization
  local dest_alphabet = idx2key(opt.dest_dict)
  local dest_alphabet = invert_table(alphabet)
  ...
  while true do
    ...
    local dest_sentence = tokenize(io.read(), inverse_dest_alphabet)
